...
import { BC_CHECKOUT_URLS, PLAN_DESCRIPTION, PLAN_FEATURES, PLAN_META, PLAN_PRICING, PlanLevel } from '../../core/plan.config';
import { SubscriptionService } from '../../core/subscription.service';

type BillingCycle = 'monthly' | 'annual';

const PAID_PLANS: Exclude<PlanLevel, 'trial'>[] = ['starter', 'pro', 'business'];
@Component({
    ...
    template: `
        <div class="upgrade-page">
            ...
            <div class="upgrade-header">
                ...
                <p class="upgrade-subtitle">Passez à la vitesse supérieure quand vous êtes prêt. Modifiez ou annulez à tout moment depuis votre admin BigCommerce.</p>

                <!-- Billing toggle -->
                <div class="billing-toggle">
                    <button class="toggle-btn" [class.active]="billingCycle() === 'monthly'" (click)="billingCycle.set('monthly')">Mensuel</button>
                    <button class="toggle-btn" [class.active]="billingCycle() === 'annual'" (click)="billingCycle.set('annual')">
                        Annuel
            ...
            <div class="plans-grid">
                @for (plan of plans; track plan) {
                    <div class="plan-card" [class.plan-card--popular]="plan === 'pro'" [class.plan-card--required]="plan === requiredPlan()" [class.plan-card--current]="plan === currentPlan()">
                        @if (plan === 'pro') {
                        ...
                        <div class="plan-header">
                            <span class="plan-label" [style.color]="meta(plan).color" [style.background]="meta(plan).bgColor">{{ meta(plan).label }}</span>
                        ...
                        <div class="plan-price">
                            ...
                            @if (billingCycle() === 'annual') {
                                ...
                                <div class="price-note">→ €{{ annualPrice(plan) }}/mois si annuel</div>
                            }
                        </div>
                        ...
                        <!-- CTA -->
                        @if (plan === currentPlan()) {
                            <p-button label="Plan actuel" styleClass="w-full" [outlined]="true" [disabled]="true" />
                        } @else if (isDowngrade(plan)) {
                            <p-button label="Gérer mon abonnement" styleClass="w-full" severity="secondary" [outlined]="true" (onClick)="openCheckout(plan)" />
                        } @else {
                            <p-button [label]="currentPlan() === 'trial' ? 'Démarrer l’essai' : 'Passer au plan ' + meta(plan).label" styleClass="w-full" [severity]="plan === 'pro' ? 'success' : 'primary'" (onClick)="openCheckout(plan)" />
                        }
                    </div>
                }
            </div>
            ...
            <div class="upgrade-footer">
                <button class="back-link" (click)="goBack()"><i class="pi pi-arrow-left"></i> Retour</button>
            </div>
        </div>
    `,
    styles: [
...
export class UpgradeComponent {
    ...
    price(plan: Exclude<PlanLevel, 'trial'>): number {
        return this.billingCycle() === 'annual' ? PLAN_PRICING[plan].annual : PLAN_PRICING[plan].monthly;
    }
