...
@Component({
    selector: 'app-transferer-lot-dialog',
    standalone: true,
    ...
    templateUrl: './transferer-lot-dialog.component.html'
})
export class TransfererLotDialogComponent implements OnChanges {
    @Input() visible = false;
    ...
    get entrepotOptions(): SelectOption[] {
        return this.entrepots.filter((e) => e.id !== this.lot?.entrepotId).map((e) => ({ label: e.nom!, value: e.id! }));
    }
    ...
    submit(ngForm: NgForm): void {
        if (ngForm.invalid || !this.lot || !this.materiau || !this.form.entrepotDestinationId || !this.form.quantite) return;
        ...
        this.materiauService
            .transfererLotMateriau(this.materiau.id!, this.lot.id!, {
