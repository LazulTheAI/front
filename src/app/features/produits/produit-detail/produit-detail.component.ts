...
export class ProduitDetailComponent implements OnChanges {
  ...
  loadProductions(): void {
    ...
    // Charger les dernières productions liées à ce produit via sa recette principale
    ...
    this.productionService.listerProductions(undefined, undefined).subscribe({
    next: (data: RunProductionResponse[]) => {
      this.productions = data
        .filter(r => r.recetteId === recettePrincipale.recetteId)
        .slice(0, 10);
      this.loadingProductions = false;
    },
    error: () => {
  ...
  openBigCommerce(): void {
    ...
    if (produitDetail?.platformProductId) {
      ...
      window.open(
        `https://store.mybigcommerce.com/manage/products/${produitDetail.platformProductId}/edit`,
        '_blank',
      );
    }
  }
