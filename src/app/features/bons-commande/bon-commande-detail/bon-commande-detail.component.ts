...
@Component({
  ...
  imports: [
    ...
    ToastModule, InputNumberModule, SelectModule,
    DatePickerModule
  ],
...
export class BonCommandeDetailComponent implements OnChanges {
  ...
  getStatutSeverity(statut: string): 'success' | 'info' | 'warn' | 'danger' | 'secondary' {
    const map: Record<string, 'success' | 'info' | 'warn' | 'danger' | 'secondary'> = {
      BROUILLON: 'secondary', ENVOYE: 'info',
      PARTIELLEMENT_RECU: 'warn', RECU: 'success', ANNULE: 'danger',
    };
  ...
  getLigneStatutSeverity(statut: string): 'success' | 'warn' | 'secondary' {
    const map: Record<string, 'success' | 'warn' | 'secondary'> = {
      EN_ATTENTE: 'secondary', PARTIELLEMENT_RECU: 'warn', RECU: 'success',
    };
    return map[statut] ?? 'secondary';
  }
  ...
  getLigneStatutLabel(statut: string): string {
    const map: Record<string, string> = {
      EN_ATTENTE: 'En attente', PARTIELLEMENT_RECU: 'Partiel', RECU: 'Reçu',
    };
    return map[statut] ?? statut;
  }
  ...
  canEnvoyer(): boolean { return this.bon?.statut === 'BROUILLON'; }
  canReceptionner(): boolean { return this.bon?.statut === 'ENVOYE' || this.bon?.statut === 'PARTIELLEMENT_RECU'; }
}
